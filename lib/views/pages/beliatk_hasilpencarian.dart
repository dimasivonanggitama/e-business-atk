...
import 'package:ebusiness_atk_mobile/views/components/custom_gridView.dart';
...
class beliatk_hasilpencarianPage extends StatefulWidget {
  const beliatk_hasilpencarianPage({Key? key}) : super(key: key);

  @override
  State<beliatk_hasilpencarianPage> createState() => _beliatk_hasilpencarianPageState();
}

class _beliatk_hasilpencarianPageState extends State<beliatk_hasilpencarianPage> with TickerProviderStateMixin {
  ...
  List<Widget> _getWidgetMenuItem({required var items, bool addIcon = false, var iconItems, var function}) {
    ...
    for (int i = 0; i < items.length; i++) {
      if (iconItems == null) localIconItems = Icons.question_mark; else localIconItems = iconItems[i];
      if (function == null) localFunction = (){}; else localFunction = function[i];
  ...
  Widget _dropDownHeader({double width = 120, bool animatedRadius = false, required String text, required IconData icon, 
      IconData secondIcon = Icons.arrow_drop_down, bool isSecondIconRotated = false, Color secondIconColor = Colors.black, 
      required void Function()? mainFunction, void Function()? secondIconFunction}) {
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        border: Border.all(), 
        borderRadius: animatedRadius? BorderRadius.only(topLeft: Radius.circular(15), topRight: Radius.circular(15)) : BorderRadius.circular(15), 
        color: Colors.white,
        boxShadow: [
          BoxShadow(
        ...
        children: [
          SizeTransition(
            ...
            child: Container(
              ...
              child: Column(
                children: [
                  for (Widget dropdownItemsWidget in _getWidgetMenuItem(items: dropdownItems, addIcon: true, iconItems: dropdownIcons, 
  ...
  Widget _floatingDropDownHeader({bool visible = false, double topPositioned = 0, double leftPositioned = 0, double width = 120, 
      bool animatedRadius = false, required String text, required IconData icon, IconData secondIcon = Icons.arrow_drop_down, 
      bool isSecondIconRotated = false, Color secondIconColor = Colors.black, required void Function()? mainFunction, 
    ...
    return Visibility(
      ...
      child: Positioned(
        ...
        child: _dropDownHeader(
          ...
          secondIconFunction: secondIconFunction
        )
      )
    );
  }
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.only(
                  bottomLeft: Radius.circular(15), 
                  bottomRight: Radius.circular(15),
                  topLeft: triggeredBy == "sort"? Radius.circular(15) : Radius.circular(0),
                  topRight: triggeredBy == "filter"? Radius.circular(15) : Radius.circular(0)
                ),
              ),
              child: Column(
                children: [
                  Align(
                    ...
                    child: Text(triggeredBy == "filter"? "Kategori" : "Berdasarkan")
                  ),
                  Padding(padding: EdgeInsets.only(bottom: 5)),
                  for (Widget dropdownItemsWidget in dropdownItems) (
                    dropdownItemsWidget
                  ),
                  Padding(padding: EdgeInsets.only(bottom: 5)),
                  GestureDetector(
  ...
  Widget _dropDownMenuItem({required String textMenu, required Color colorMenu, bool addIcon = false, IconData icon = Icons.question_mark, 
                  ...
                  delegate: SliverChildBuilderDelegate(
                    (context, index) {
                      ...
                      return _item_regularShape(
                        ...
                        fotoProduk: "${snapshot.data!.docs[index]['gambarProduk']}"
                      );
                    },
                    // childCount: 2,
